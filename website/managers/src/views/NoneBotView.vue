          ...
          <template v-if="activeTab === 'commands'">
            ...
            >
              <div class="flex items-center justify-between gap-2">
                ...
                <span class="shrink-0 rounded-full px-2 py-0.5 text-[11px]" :class="commandItem.runtime_loaded ? okChipClass : mutedChipClass">
              ...
              <div class="mt-1 flex items-center gap-2 text-[11px] text-on-surface-variant dark:text-zinc-500">
                ...
                <span>{{ commandItem.matcher_type }}</span>
              </div>
            </button>
          ...
          <template v-else-if="activeTab === 'plugins'">
            ...
            >
              <div class="flex items-center justify-between gap-2">
                ...
                <span class="shrink-0 rounded-full px-2 py-0.5 text-[11px]" :class="plugin.loaded ? okChipClass : mutedChipClass">
                  {{ plugin.loaded ? 'loaded' : plugin.source }}
                </span>
              </div>
              ...
              <div class="space-y-4">
                <DetailCard>
                  <div class="flex flex-wrap items-center gap-2">
                    <span :class="selectedCommand.matcher_type === 'alconna' ? infoChipClass : mutedChipClass">{{ selectedCommand.matcher_type }}</span>
                    <span :class="selectedCommand.documented ? okChipClass : warnChipClass">{{ selectedCommand.documented ? '帮助已登记' : '仅源码识别' }}</span>
                    <span :class="selectedCommand.runtime_loaded ? okChipClass : mutedChipClass">{{ selectedCommand.runtime_loaded ? '运行时已载入' : '未确认载入' }}</span>
                  </div>
                ...
                <DetailCard title="命令参数">
                  <div v-if="selectedCommand.params.length" class="flex flex-wrap gap-2">
                    <span
                      v-for="param in selectedCommand.params"
                      :key="param.name"
                      class="rounded-lg border border-outline-variant bg-surface-container px-2 py-1 text-[12px] text-on-surface dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"
                    >
                      {{ param.name }}{{ param.mode || '' }}
                    </span>
                  </div>
                ...
                <DetailCard title="源码定位">
                  <div class="grid gap-2 font-code-inline text-code-inline text-on-surface-variant dark:text-zinc-400">
                    <span>{{ selectedCommand.file }}:{{ selectedCommand.line }}</span>
...
import { fetchNoneBotOverview } from '@/api/nonebot'
import type {
  NoneBotAdapterItem,
  NoneBotBotItem,
  NoneBotCommandItem,
...
const detailSubtitle = computed(() => {
  if (activeTab.value === 'commands') return selectedCommand.value ? `${selectedCommand.value.plugin_module} · ${selectedCommand.value.file}` : '选择命令查看源码与权限元数据'
