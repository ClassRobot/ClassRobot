...
class AgentLocalKnowledgeRetriever:
    ...
    async def retrieve(self, query: str, context: AgentRuntimeContext | None) -> str | None:
        """根据用户问题和运行时上下文检索本地知识。"""
    ...
    async def retrieve_chat_history(self, query: str, context: AgentRuntimeContext) -> str | None:
        """检索用户聊天流和已绑定系统群的采集消息。"""
    ...
    async def retrieve_user_chat_rag(self, query: str, context: AgentRuntimeContext) -> str | None:
