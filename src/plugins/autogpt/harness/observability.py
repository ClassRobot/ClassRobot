...
from ..schema import AgentObservabilityMetrics, ObservabilityStage, PromptStageMetric
...
class ProgressFeedbackHarness:
    ...
    def record_prompt_stage(
        ...
        *,
        ...
        normalized_commands = self._normalize_names(recalled_commands)
        normalized_skills = self._normalize_names(recalled_skills)
        normalized_selected_commands = self._normalize_names(selected_commands)
    ...
    def record_planner_candidate_commands(self, commands: Sequence[str]) -> None:
        ...
        self._metrics.planner_candidate_commands = self._normalize_names(commands)

    def record_final_hit_commands(self, commands: Sequence[str]) -> None:
        """记录最终通过校验、可真正执行的命令集合。"""

        self._metrics.final_hit_commands = self._normalize_names(commands)
