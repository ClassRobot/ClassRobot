...
class WorkflowCheckpointStore:
    ...
    async def load_workflow(self, user_id: int) -> AgentWorkflow | None:
        ...
        checkpoint = await self._get_checkpoint(user_id)
        if checkpoint is None:
            return None
        return self._parse_workflow(checkpoint.workflow_data, user_id=user_id, trace_id=checkpoint.trace_id)
    ...
    async def load_pending_workflow(self, user_id: int) -> AgentWorkflow | None:
        ...
        if checkpoint is None or checkpoint.status != "needs_confirm":
            return None
        workflow = self._parse_workflow(checkpoint.workflow_data, user_id=user_id, trace_id=checkpoint.trace_id)
    ...
    async def _get_checkpoint(self, user_id: int) -> AgentWorkflowCheckpoint | None:
        """按用户读取最新检查点。"""
    ...
    @staticmethod
    def _parse_workflow(payload: dict, user_id: int, trace_id: str) -> AgentWorkflow | None:
        """从 JSON 快照恢复工作流对象。"""
