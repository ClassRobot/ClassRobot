...
# --------------------------------- 添加德育 ---------------------------------
@add_moral.handle()
async def _(matcher: Matcher, state: T_State, msg: Message = CommandArg(), student: StudentInfo = STUDENT):
    moral = AddMoral(student)
    moral.save_file.add_file(msg)
    state["moral"] = moral
    if text := msg.extract_plain_text().strip():
        if not moral.is_cader:
...
# --------------------------------- 导出德育 ---------------------------------
@export_moral.handle()
async def _(matcher: Matcher, state: T_State, event: MessageEvent, msg: Message = CommandArg(), user: User = USER):
    moral = ExportMoral(user)
    if isinstance(user, StudentInfo):
        await upload_file(
            event, 
            await moral.export_moral(user.class_field, msg.extract_plain_text().strip())
        )
...
async def _(matcher: Matcher, state: T_State, event: MessageEvent, class_name: Message = Arg()):
        ...
        if class_table := await moral.select_class(text):
            await upload_file(
                ...
                await moral.export_moral(
                    class_table, 
                    state["export_date"].msg.extract_plain_text().strip()
                )
            )
            await matcher.finish()
