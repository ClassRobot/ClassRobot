...
from .method import BatchImportStudents, AddStudent, base_info
...
async def _(matcher: Matcher, _: GroupMessageEvent, state: T_State, params: str = ArgPlainText()):
    ...
    try:
        if sheet := await GetDocsSheet(url):
            ...
            if columns := bis.check_base_columns():
                ...
                await matcher.finish(",".join(str(i) for i in missing.index + 1) + "有东西没写！")
