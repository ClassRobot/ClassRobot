...
class FindUser(BaseFindUser):
    ...
    def display_columns(self) -> List[str]:
        ...
        if self._columns is None:
            ...
            if self.fs.strings:
                for key in (
                    i
                    for i in self.show_keys
                    if i in self.fs.strings and i not in self._columns
                ):
    ...
    async def find_user(self, class_table: ClassTable):
        ...
        return "\n===\n".join(
            i
            for i in (
                self.student_to_string(students),
                self.teacher_to_string(teachers),
            )
            if i
        )
    ...
    def to_string(self, data: DataFrame) -> str:
        return "\n---\n".join(
            "\n".join(f"{k}:{v}" for k, v in i.items()) for i in data.to_dict("records")
        )
