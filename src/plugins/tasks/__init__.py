...
from .methods import AddTask, DelTask, PushTask, ShowTask, ClearTask, ExportTask
...
async def _(matcher: Matcher, state: T_State, class_name: Message = Arg()):
    ...
    if class_table := await task.select_class(class_name.extract_plain_text()):
        if class_task := await task.add_task(
            state["task_title"], class_table=class_table
        ):
            await matcher.finish(f"OK,任务ID：{class_task.id}")
        await matcher.finish("似乎重名了！")
    else:
...
@del_task.handle()
async def _(
    state: T_State, matcher: Matcher, msg: Message = CommandArg(), user: User = USER
):
    task = DelTask(user)
    state["task"] = task
    if not await task.exists():
        ...
        await matcher.send(
            MessageSegment.image(await task.tasks_to_image("请输入您要删除的任务的ID"))
        )
...
async def _(matcher: Matcher, msg: Message = CommandArg(), user: User = USER):
    ...
    if not await task.exists():
        ...
        await matcher.finish(MessageSegment.image(await task.tasks_to_image()))
...
async def _(
    ...
    if not await task.exists():
        ...
        await matcher.send(
            MessageSegment.image(await task.tasks_to_image("输入需要提交的任务ID"))
        )
...
async def _(
    ...
    if not await task.exists():
        ...
        await matcher.send(MessageSegment.image(await task.tasks_to_image("需要导出哪个任务")))
    ...
    if isinstance(user, Student):  # 学生进行删除
        ...
        await matcher.send(
            ...
            + (
                "\n".join(
                    f"{i}.{v}"
                    for i, v in enumerate([i.name async for i in task.teacher_class])
                )
            )
        )
