...
# --------------------------------- 群内同学重命名 ---------------------------------
@rename.handle()
async def _(matcher: Matcher, bot: Bot, event: GroupMessageEvent, msg: Message = CommandArg(), _: User = USER):
    bot_info = await bot.get_group_member_info(group_id=event.group_id, user_id=int(bot.self_id))
    if bot_info["role"] != "member":
        text = msg.extract_plain_text()
        _rename = [rename_columns[i] for i in text.split() if i in rename_columns] or ['name', 'dormitory', 'phone']
        if not _rename:
        ...
        for name, user_id in ((i["card"], i["user_id"]) for i in await bot.get_group_member_list(group_id=event.group_id)):
            if user := await Student.objects.filter(qq=user_id).values(*_rename).afirst():
                ...
                if newname not in name:
                    await sleep(3)
                    await bot.set_group_card(group_id=event.group_id, user_id=user_id, card=newname)
...
async def _(matcher: Matcher, state: T_State, images: Message = Arg()):
    if images := images.get("image"):
        ...
        await matcher.finish(
            Message(
                [MessageSegment.image(image) async for image in w.watermark(
                    extract_image_urls(images)
                )]
            )
        )
