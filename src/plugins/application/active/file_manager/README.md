...
文件管理插件负责把个人、群组、班级、学院、学校文件空间汇总成一个对用户友好的“文件管理”视图。实际存储根目录来自 `src.platform.config.data_dir`，最终结构固定为 `{data_dir}/storage`。
...
- `src.core.storage` 是核心隔离文件空间层，不依赖 NoneBot，适合被后台管理、Agent service handler 和测试复用。
- `src/plugins/application/active/file_manager/virtual.py` 是虚拟挂载层，负责身份挂载、路径解析和写权限判断。
- `src/plugins/application/active/file_manager/commands.py` 只放命令声明和统一命令元数据。
- `src/plugins/application/active/file_manager/__init__.py` 只放 matcher handler。
- `src/plugins/application/active/file_manager/services.py` 放命令层服务、附件提取、Agent service handler 和展示渲染。
