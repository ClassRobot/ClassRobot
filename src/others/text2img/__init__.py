...
status_cmd = on_command("设置绘画", aliases={"设置画画"}, priority=100, block=True, permission=SUPERUSER)
...
def draw_image(is_translate: bool = False):
    async def _(bot: Bot, matcher: Matcher, event: MessageEvent, msg: Message = CommandArg()):
            ...
            if text := msg.extract_plain_text():
                if any(i in text for i in ["nsfw", "naked"]):
                    ...
                    try:
                        ...
                        await matcher.send("开始画画啦！" + (f"\n{text}" if is_translate else ""))
                        image = MessageSegment.image(await tti.draw(text, extract_image_urls(msg)))
