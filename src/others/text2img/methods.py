        ...
        try:
            return (
                await self.loop.run_in_executor(
                    None, self.translate.translate, text, "en"
                )
            ).translatedText
        except Exception as err:
            logger.exception(err)
            return (
                await self.client.post(self.translate_url, params={"msg": text})
            ).text

    async def draw(
        self, text: str, from_image_url: Optional[List[str]] = None
    ) -> bytes:
        from_image = (
            b64encode((await self.client.get(from_image_url[0])).content).decode()
            if from_image_url
            else None
        )
        response = await self.client.post(
            self.main_url, json=BetaParams(text) | FromImageParams(from_image)
        )
