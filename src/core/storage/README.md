...
- 路径空间
  - `users/{user_id}`、`groups/{group_id}` 等目录是物理承载边界。
- owner scope
  - Agent 和本地 RAG 实际可读取谁，由上层 runtime 解析出的 `private_user` / `bound_group` 决定。

也就是说，路径不能替代权限语义。平台 user_id、平台群号、频道号只能用于绑定解析，不能直接作为聊天检索或本地 RAG 读取目标。
...
补充当前 `group_chat_history` 契约：

- 群用户发言来自 `collect`
- 机器人在该群的回复来自 `chat`
- 已经由 `collect` 覆盖的人类命令回放，不应在群聊天历史视图和本地 RAG 中重复出现
...
- `LocalRagService` 只支持按 owner 读取单个 `user` 或 `group` 空间，不提供跨 owner 默认聚合搜索入口。
- 文件索引只处理小型文本文件，跳过隐藏目录和二进制文件，避免把用户不可读或过大的内容塞进上下文。
- 聊天索引与普通聊天检索共享 owner 边界和 `exclude_message_id` 规则，避免当前消息自回声和跨用户串读。
