...
`src/core/cache` 是项目当前对缓存能力的统一入口。

如果你现在要做的是“怎么配置缓存”，先看 [配置说明](../../../docs/getting-started/configuration.md)；如果你要看的是“代码里如何安全使用缓存”，本文更适合。
...
当前仓库里，`src/core/cache` 主要用于这几类场景：
...
- `src/plugins/application/active/user/__init__.py`
- `src/plugins/application/active/curriculum/__init__.py`
...
```mermaid
flowchart TD
    A["业务代码"] --> B["src.core.cache.get_cache(...)"]
    B --> C["CacheClient"]
...
```python
from src.core.cache import set
...
```python
from src.core.cache import get
...
```python
from src.core.cache import get_and_delete
...
当前如果你使用包装函数 `src.core.cache.set(...)`，建议始终显式传入 TTL：
...
1. 先看 `src/core/cache/__init__.py`
2. 再看 `src/core/cache/config.py`
3. 然后看 `src/plugins/application/active/user/__init__.py` 的 token 流程
