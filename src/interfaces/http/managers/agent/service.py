...
AGENT_MODULES: tuple[AgentModuleDefinition, ...] = (
    ...
    AgentModuleDefinition(
        ...
        description="统一封装用户 turn、上下文包、运行时角色记录和用户可见输出策略。",
        capabilities=("TurnEnvelope", "ContextPack", "Runtime role trace", "Reply envelope"),
    ...
    AgentModuleDefinition(
        id="runtime_role_catalog",
        name="RuntimeRoleCatalog",
        category="运行时角色",
        source="src/core/agent/runtime/roles",
        description="声明 WorkflowSupervisor、Conversation、Knowledge、RealtimeLookup、Execution 和 ReplySynthesis 等运行时角色。",
        capabilities=("运行时角色目录", "上下文切片", "Role trace", "执行边界"),
        control_note="角色由 AgentHost 统一审计，不代表真实 BaseAgent 子智能体。",
    ),
    AgentModuleDefinition(
        id="context_engine",
        name="ContextEngine",
