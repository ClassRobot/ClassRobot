...
def _get_manager_driver() -> Any:
    """返回缓存的 NoneBot Driver，避免模块导入阶段依赖 NoneBot 初始化。"""
...
def _dump_driver_config() -> dict[str, Any]:
    ...
    """
    config = _get_manager_driver().config
...
def get_runtime_config_snapshot() -> dict[str, Any]:
    ...
    return {
        "driver": current_driver.__class__.__name__,
        "config_model": current_driver.config.__class__.__name__,
...
def _validate_model_configs(value: Any, field: str) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(value):
        ...
        if config.proxy and not config.proxy.startswith(VALID_PROXY_SCHEMES):
            schemes = ", ".join(VALID_PROXY_SCHEMES)
            raise ValueError(f"Setting `{field}[{index}].proxy` must start with one of: {schemes}")
