...
- `core.storage` 是核心隔离文件空间层，不依赖 NoneBot，适合被后台管理、Agent service handler 和测试复用。
- `src/features/file_manager/virtual.py` 是虚拟挂载层，负责身份挂载、路径解析和写权限判断。
- `src/features/file_manager/commands.py` 只放命令声明和统一命令元数据。
- `src/features/file_manager/__init__.py` 只放 matcher handler。
- `src/features/file_manager/services.py` 放命令层服务、附件提取、Agent service handler 和展示渲染。
