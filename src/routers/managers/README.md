...
`src/routers/managers/` 当前是管理端实现层的兼容路径；新的 canonical import 位于 `src/interfaces/http/manager/`。这里仍统一承载 `/api/v1/manager/*` 相关路由、服务编排和返回结构。
...
- `__init__.py`：兼容历史导入路径，以懒加载方式把旧模块名映射到新的领域子包实现
...
- 若历史代码仍引用 `src.routers.managers.xxx` 旧路径，先复用 `__init__.py` 的懒加载兼容映射；新增代码直接使用新的子包路径
