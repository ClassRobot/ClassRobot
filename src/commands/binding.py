...
from utils.helper import Context, ParamMode, HelperScope
...
class CommandBinding(BaseModel):
    ...
    execution_mode: CommandExecutionMode = "legacy_event"
...
def command_command(command: str, *, binding: CommandBinding | None = None, aliases=None, **kwargs):
    ...
    该包装器主要用于简单命令或暂时无法迁移到 Alconna 的旧命令。
...
def _caller_module_name() -> str | None:
    ...
    for frame_info in stack()[2:]:
        ...
        if not isinstance(module_name, str):
            continue
        if module_name.startswith("src.commands"):
            continue
        return module_name
    return None
