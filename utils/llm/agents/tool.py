...
class AgentTool:
    ...
    def from_function(
        ...
        *,
        ...
        parameters, model_param = _build_parameters(func)
        return cls(
            name=tool_name,
...
def _build_parameters(func: ToolHandler) -> tuple[JsonSchema, str | None]:
...
def _annotation_to_schema(annotation: Any) -> JsonSchema:
    ...
    if origin in {Union, UnionType}:
        ...
        schemas = [_annotation_to_schema(arg) for arg in args if arg is not type(None)]
    ...
    if origin is Literal:
        ...
        values = list(args)
        literal_schema: JsonSchema = _annotation_to_schema(type(values[0])) if values else {"type": "string"}
        literal_schema["enum"] = values
        return literal_schema
    if origin in {list, tuple, set}:
        item_schema: JsonSchema = _annotation_to_schema(args[0]) if args else {"type": "string"}
        return {"type": "array", "items": item_schema}
    if origin is dict:
        return {"type": "object"}
    if inspect.isclass(annotation) and issubclass(annotation, Enum):
        values = [item.value for item in annotation]
        enum_schema: JsonSchema = _annotation_to_schema(type(values[0])) if values else {"type": "string"}
