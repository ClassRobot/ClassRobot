...
class User(Base):
    ...
    teacher: Mapped[List["Teacher"]] = relationship(
        "Teacher", uselist=True, back_populates="user"
    )
...
class Student(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(["user_id"], ["user.id"], name="student_ibfk_2"),
        Index("class_table", "class_table"),
        Index("email", "email", unique=True),
        Index("id", "id", unique=True),
    ...
    moral_education: Mapped[List["MoralEducation"]] = relationship(
        "MoralEducation", uselist=True, back_populates="student_"
    )
...
class MoralEducation(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["student"], ["student.id"], name="moral_education_ibfk_2"
        ),
        Index("class_table", "class_table"),
        Index("id", "id", unique=True),
    ...
    class_table = mapped_column(Integer, nullable=False, comment="班级id")
    student = mapped_column(Integer, nullable=False, comment="学生id")
    description = mapped_column(
...
class TaskFiles(Base):
    ...
    user_id = mapped_column(Integer, nullable=False, comment="提交人QQ")
