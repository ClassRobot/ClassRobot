...
class Encoder:
    ...
    def encode(self, value: EncodableT) -> EncodedT:
        ...
        if not isinstance(value, str):
            ...
            raise DataError(f"Invalid input of type: {typename!r}. " "Convert to a bytes, string, int or float first.")
...
class UnixDomainSocketConnection(Connection):  # lgtm [py/missing-call-to-init]
    ...
    def _error_message(self, exception):
        ...
        if len(exception.args) == 1:
            ...
            return f"Error {exception.args[0]} connecting to unix socket: " f"{self.path}. {exception.args[1]}."
    ...
    async def disconnect(self, inuse_connections: bool = True):
        ...
        async with self._lock:
            if inuse_connections:
                connections: Iterable[Connection] = chain(self._available_connections, self._in_use_connections)
            else:
