...
1. Use `utils.skills.ocr_skill.read_code(...)` for验证码或单行短文本。
2. Use `utils.skills.ocr_skill.read_lines(...)` for multi-line text detection and recognition.
3. Use `utils.skills.ocr_skill.read_text(...)` when you want one combined text result with automatic fallback.
