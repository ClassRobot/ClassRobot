...
`core.agent` 是 ClassRobot 唯一的 Agent 标准入口。以后只有继承 `BaseAgent` 或 `BaseFunctionAgent` 的对象，才允许命名为 `Agent`。
...
from core.agent import BaseAgent, BaseAgentConfig
...
开发者如果要改“消息先经过哪些阶段”，应改 `core.agent.runtime.orchestration_config` 和管理端图配置。如果要改“某个阶段如何推理、抽取、总结”，才新增或修改 `BaseAgent` 子类。
